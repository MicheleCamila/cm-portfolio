...
// isUpsert=true sets the Prefer header needed for ON CONFLICT upserts
async function supabase(method, path, body, isUpsert) {
  ...
  // Prefer header rules:
  //   upsert  → return=representation + resolution=merge-duplicates
  //   POST/PATCH → return=representation  (so we get the row back)
  //   GET/DELETE → not needed
  let prefer = '';
  if (isUpsert) {
  ...
  // Supabase returns 204 No Content for DELETEs — handle empty body gracefully
  ...
  if (text && text.trim()) {
    try {
      ...
      throw new Error('Invalid JSON from Supabase: ' + text.slice(0, 120));
    }
  }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'save_cv' && req.method === 'POST') {
      ...
      // true = upsert (insert if id=1 doesn't exist, update if it does)
    ...
    // SAVE SETTING 
    // The password is hashed in the browser before arriving here.
    // The raw password is NEVER sent to the server.
